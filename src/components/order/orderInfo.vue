                                    ...
                                    <div class="order-progress">
                                        <ul>
                                            ...
                                            <li :class="orderInfo.status >=2?'active':''">
                                                ...
                                                <div class="info" v-if="orderInfo.status >=2">{{orderInfo.payment_time | dateFmt("YYYY-MM-DD HH:mm:ss")}}</div>
                                            </li>
                                            <li :class="orderInfo.status >=3?'active':''">
                                                <div class="progress">已经发货</div>
                                                <div class="info" v-if="orderInfo.status >=3">{{orderInfo.express_time | dateFmt("YYYY-MM-DD HH:mm:ss")}}</div>
                                            </li>
                                            <li :class="['last',orderInfo.status >=4?'active':'']">
                                                <div class="progress">已完成</div>
                                                <div class="info" v-if="orderInfo.status >=4">{{orderInfo.complete_time | dateFmt("YYYY-MM-DD HH:mm:ss")}}</div>
                                            </li>
                                        </ul>
                                    </div>
                                    <div class="form-box accept-box form-box1">
                                        <dl class="head form-group">
                                            <dd>
                                                ...
                                                <router-link :to="'/pay?orderid='+orderInfo.id" v-if="orderInfo.status===1" class="btn-pay">去付款</router-link>
                                                <!---->
                                            </dd>
                                        </dl>
                                        <dl class="form-group">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td colspan="7" align="right">
                                                        <p>商品金额：
                                                            <b class="red">￥{{goodsprice}}</b>&nbsp;&nbsp;+&nbsp;&nbsp;运费：
                                                            <b class="red">￥{{expressprice}}</b>
                                                        </p>
                                                        <p style="font-size: 22px;">应付总金额：
                                                            <b class="red">￥{{allprice}}</b>
                                                        </p>
                                                    </td>
                                                </tr>
                                            </tbody>
        ...
        methods:{
            getorderinfo(){
                ...
                this.$axios.get(url).then(res=>{
                    ...
                    this.expressprice=res.data.message.orderinfo.express_fee
                    this.goodsList=res.data.message.goodslist
                    res.data.message.goodslist.forEach(ele => {
                        this.goodsprice+=ele.quantity*ele.real_price
                    });
                    this.allprice=this.expressprice+ this.goodsprice
                })
            }
        }
    }
